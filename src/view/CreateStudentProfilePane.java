...
public class CreateStudentProfilePane extends GridPane {
	...
	public void addCourseDataToComboBox(Set<Course> courses) {
	    ...
	    if (!courses.isEmpty()) {
	        cboCourses.getSelectionModel().select(0); //select the first course by default
	        System.out.println("Courses added to ComboBox: " + courses);
	    } else {
	        System.out.println("No courses available to add to ComboBox.");
	    }
	}
    ...
    // method to create a StudentProfile based on the selected values
    public StudentProfile createStudentProfile() {
        StudentProfile studentProfile = new StudentProfile();
        studentProfile.setStudentPnumber(getStudentPnumber());
        studentProfile.setStudentName(getStudentName());
        studentProfile.setStudentEmail(getStudentEmail());
        studentProfile.setSubmissionDate(getStudentDate());
        studentProfile.setStudentCourse(getSelectedCourse());
